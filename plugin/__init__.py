...
def _validate_config():
    ...
    from .tools import _validate_configured_agents
    _validate_configured_agents(cfg.get("a2a", {}).get("agents", []))
...
def _bootstrap_friends_from_legacy_token() -> None:
    """Migrate ``A2A_AUTH_TOKEN`` to a ``legacy`` friend on first startup.

    Called at register(). Behaviour:

    - If FriendsStore is empty and ``A2A_AUTH_TOKEN`` is set: import the
      env token as a single ``legacy`` friend (status=active,
      trust_level=normal). Existing callers continue to work transparently
      because their bearer matches the legacy friend's hash.
    - If FriendsStore already has any record and ``A2A_AUTH_TOKEN`` is still
      set: log a one-time startup warning telling the maintainer to manage
      access through friends and remove the env var. The env var is no
      longer consulted by auth decisions.
    - If neither condition applies: silent.
    """
    from .friends import friends as friends_store

    legacy_token = os.getenv("A2A_AUTH_TOKEN", "")
    has_friends = bool(friends_store.list_friends())

    if not has_friends and legacy_token:
        result = friends_store.bootstrap_legacy(legacy_token)
        if result is not None:
            logger.info(
                "[A2A] Migrated A2A_AUTH_TOKEN to FriendsStore as 'legacy' friend "
                "(id=%s). Existing callers using the env token continue to work; "
                "manage future access via the FriendsStore.",
                result["id"],
            )
    elif has_friends and legacy_token:
        logger.warning(
            "[A2A] A2A_AUTH_TOKEN is still set in the environment but FriendsStore "
            "is the source of truth. The env var is NOT consulted for auth "
            "decisions. Remove A2A_AUTH_TOKEN from .env once you are confident "
            "all callers have migrated to per-friend tokens."
        )
...
def register(ctx):
    ...
    _validate_config()
    ssrf.log_env_state()
    _bootstrap_friends_from_legacy_token()
...
def _handle_a2a_command(raw_args: str) -> str:
    ...
    if sub == "friends" or sub.startswith("friends "):
        from .cli import handle_friends_command
        return handle_friends_command(raw_args.strip()[len("friends"):].strip())
    if sub == "agents":
        return _cmd_agents()
    return _cmd_status()
...
def _activate_task_if_idle(task) -> bool:
    ...
    with _active_tasks_lock:
        ...
        metadata = provenance.attach_internal_provenance(
            task.metadata,
            provenance.trusted_from_metadata(task.metadata, required=True),
        )
...
def _merge_active_task_provider_provenance(task_id: str, provider_metadata: dict | None, *, provider: str = "") -> bool:
    """Merge a private source provider hook's provenance into an active A2A task.

    Future MEMORY/DIARY/inbox/wakeup hooks should call this after writing their
    provider provenance. Missing provider provenance deliberately becomes
    unknown_private so the outbound path fails closed.
    """
    with _active_tasks_lock:
        ...
        merged = provenance.merge_provenance(
            provenance.trusted_from_metadata(metadata, required=True),
            source_providers.trusted_provider_provenance(provider_metadata, provider=provider, required=True),
        )
        entry["metadata"] = provenance.attach_internal_provenance(metadata, merged)
        return True
...
def _on_pre_gateway_dispatch(event=None, **kwargs):
    ...
    if not pending:
        # Stale or duplicate trigger — no pending task to drain. Drop instead
        # of falling through, so the synthetic "[A2A trigger]" never reaches
        # the user-visible session.
        return {"action": "skip", "reason": "no pending A2A task"}
